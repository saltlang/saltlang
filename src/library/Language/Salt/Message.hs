...
--import qualified Data.ByteString.Strict.UTF8 as Strict.UTF8
...
instance Msg.Message Message where
  ...
  brief CannotFind { cannotFindName = cname, cannotFindIsFileName = True } =
    Lazy.concat [ Lazy.UTF8.fromString "File \"", Lazy.fromStrict cname,
                  Lazy.UTF8.fromString "\" does not exist" ]
  brief CannotFind { cannotFindName = cname, cannotFindIsFileName = False } =
    Lazy.concat [ Lazy.UTF8.fromString "Cannot locate component ",
                  Lazy.fromStrict cname ]
  brief CannotAccess { cannotAccessName = Nothing,
                       cannotAccessFileName = fname } =
    Lazy.concat [ Lazy.UTF8.fromString "Cannot access file \"",
                  Lazy.fromStrict fname, Lazy.UTF8.fromString "\"" ]
  brief CannotAccess { cannotAccessName = Just cname } =
    Lazy.concat [ Lazy.UTF8.fromString "Cannot access component ",
                  Lazy.fromStrict cname ]
  ...
  details CannotAccess { cannotAccessName = Nothing,
                         cannotAccessMsg = msg } =
    Lazy.concat [ Lazy.UTF8.fromString "Error while accessing file:",
                  Lazy.fromStrict msg ]
